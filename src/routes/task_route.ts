...
async function getDependencies(rootDir: string): Promise<string> {
    try {
      ...
      const poetryContent = await readFile(path.join(rootDir, "poetry.toml"), "utf8")
        .then((file) => {
...
async function generateQnas(
    ...
    try{
        return new Promise<QuestionAnswer[]>((resolve, reject)=>{
            ...
            .then(async(llm_answer) => {
              logger.debug(`Raw question response: ${JSON.stringify(llm_answer.text)}`);
              var questionsArr: string[] = llm_answer.text.split("\nQuestion")
              ...
              var answersArr:ChainValues[] = await Promise.all(answerPromises)
              logger.debug(`Here are the raw answers: ${JSON.stringify(answersArr)}`)
              ...
              logger.info(`Here are the questions and answers:\n${JSON.stringify(qnaPairs)}`);
              resolve(qnaPairs)
          }else{
        ...
        return new Promise<QuestionAnswer[]>((res, rej)=>{rej(err)})
    }
   ...
   * @param qnaPairs 
   */
 async function generateTask(task:Task,
    qnaPairs:QuestionAnswer[],
    repoTree:object, pastTasksPass:string[], 
    pastTasksFail:string[]):Promise<string>{
    ...
    try{
        return new Promise<string>((resolve, reject)=>{
           taskChain.call({
                ...
                var nextTask = llm_res.text.split("Task:")[1]
                resolve(nextTask)
              }).catch(err=>{
        ...
        return new Promise<string>((res,rej)=>{rej()})
    }
    ...
    tg_router.post(
        ...
        async(req: Request, res: Response) => {
            ...
            Promise.all([
                  ...
                  getDependencies(
                    `./repos/${task.owner}/${task.repo}/${task.baseBranch}`,
                  ),
                ]).then(async ([treeResponse, pastTasksPassed, pastTasksFailed, dependencies])=>{
                    const repoTree = treeResponse.data
             if (dependencies.length > 0) {
                await writeFile(`./repos/${task.owner}/${task.repo}/${task.branchName}/dependencies.txt`, dependencies);
              }
                generateQnas(task,repoTree,pastTasksPassed, pastTasksFailed, dependencies).then(qnaPairs=>{
                    ...
                    res.status(500).json({message:`Error generating qna pairs: ${err}`})
                })
...
tg_router.post('/genTask', v.validateTask, async(req:Request, res:Response)=>{
    const task = req.body.task
    ...
    axios(qnaConfig).then(response=>{
        logger.debug("Qnas got now moving on to task...")
        if(response.data){
            var {pairs,tree,ptp,ptf} = response.data
            generateTask(
                task,pairs,tree,ptp,ptf
            ).then(response=>{
                logger.debug(`Task: ${response}`)
                res.status(200).json({message:`Task successfully generated`, data:response})
            }).catch(err=>{
        ...
        res.status(500).json({message:`Error generating task, could not make qna pairs: ${err}`})
    })
})