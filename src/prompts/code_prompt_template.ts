  ...
  async function load_prompts():Promise<PromptArray>{
    const PREFIX = await readFile(
      "src/prompts/code_gen_prefix.md",
      "utf8",
    );
    const format_prompt = await readFile("src/prompts/code_gen_format.md", "utf8")
  ...
  export default class CodePromptTemplate extends BaseChatPromptTemplate {
    ...
    async formatMessages(values: InputValues): Promise<BaseMessage[]> {
      ...
      const {
        0:{previousCode}, 
        1:{codeError}, 2:{recentLogs}, 
        3:{description}, 4:{tree}, 5:{feedback}} = values
      const result = {previousCode, codeError, recentLogs, description, tree, feedback}
      console.log(JSON.stringify(values))
      if(result.description.length == 0){
        throw new Error("No description!")
      }
      
      const formatted = renderTemplate(
        template, 
        "f-string", 
        result);
    ...
    partial(_values: PartialValues): Promise<BasePromptTemplate> {
        console.log(`Partial values: ${_values}`)
